...
namespace Prograde::Builders{
	...
	InstanceBuilder& InstanceBuilder::setApplicationVersion(uint32 variant, uint32 major, uint32 minor, uint32 patch){
		_applicationVersion = VK_MAKE_API_VERSION(variant, major, minor, patch);
		return *this;
	}

	InstanceBuilder& InstanceBuilder::setEngineVersion(uint32 variant, uint32 major, uint32 minor, uint32 patch){
		_engineVersion = VK_MAKE_API_VERSION(variant, major, minor, patch);
		return *this;
	}

	InstanceBuilder& InstanceBuilder::setVulkanAPIVersion(uint32 version){
		_apiVersion = version;
		return *this;
	}
	...
	std::shared_ptr<Instance> InstanceBuilder::build(VkAllocationCallbacks* callbacks){
		...
		try{
			...
			RAINDROP_log(ERROR, GRAPHICS, "vulkan instance creation failed !");
			throw e;
		}
		
		RAINDROP_log(INFO, GRAPHICS, "vulkan instance creation success !");
		return instance;
	}
	...
	void InstanceBuilder::checkExtensions(){
		...
		// if (!extensions.empty()){
		// 	throw MissingExtension(*extensions.begin());
		// }
	}
	...
	InstanceBuilder& InstanceBuilder::requireGraphicsExtensions(){
		...
		if (!SDL_WasInit(SDL_INIT_VIDEO)){
			if (SDL_Init(SDL_INIT_VIDEO) != 0){
				...
				errMessage << "failed to initialize the SDL2 API : " << SDL_GetError();
